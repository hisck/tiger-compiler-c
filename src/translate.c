...
// The level of library functions and global variables
static Tr_level outer = NULL;
/**
 * Each call returns the same level,
 * which is the level of library functions and
 * global variables
 */
...
static Tr_accessList make_formals(Tr_level level) {
  ...
  /**
   * Ignore head node, do not need to add static link. The translate module
   * only needs to tell frame to add a static link when new frame. You don’t
   * need to deal with static link.
   **/
...
Tr_level Tr_newLevel(Tr_level parent, Temp_label name, U_boolList formals) {
  ...
  level->parent = parent;
  level->name = name;
  // Add one more to the original formals as a static link
...
Tr_exp Tr_simpleVar(Tr_access access, Tr_level level) {
  ...
  // F_formals returns all the formals in the frame, in which the head is static
  // link
...
Tr_exp Tr_fieldVar(Tr_exp field, int offset) {
  return Tr_Ex(
      T_Mem(T_Binop(T_plus, unEx(field), T_Const(offset * F_WORD_SIZE))));
}

Tr_exp Tr_subscriptVar(Tr_exp array, Tr_exp index) {
  return Tr_Ex(T_Mem(T_Binop(
      T_plus, unEx(array), T_Binop(T_mul, unEx(index), T_Const(F_WORD_SIZE)))));
}
...
Tr_exp Tr_nilExp() {
  if (!nil) {
    ...
    return Tr_Ex(
        T_Eseq(T_Move(T_Temp(nil), F_externalCall(String("initRecord"),
...
/**
 * Literal string is stored in the heap as a global string in other languages.
 * Here a fraglist is maintained to represent the global string, and a label is
 * generated for each string, that is, the memory address
 */
...
Tr_exp Tr_arithExp(A_oper op, Tr_exp left, Tr_exp right) {
  ...
  switch (op) {
    ...
    case A_divideOp:
      ...
      break;
    default:
      printf("%s:%d arithExp cannot handle other operators", __FILE__,
             __LINE__);
  }
...
Tr_exp Tr_recordExp(Tr_expList fields, int n_fields) {
  ...
  T_stm seq =
      T_Move(T_Mem(T_Binop(T_plus, T_Temp(t), T_Const(0))), unEx(head->head));
  for (int i = 1; i < n_fields; i++) {
    head = head->tail;
    seq = T_Seq(
        T_Move(T_Mem(T_Binop(T_plus, T_Temp(t), T_Const(i * F_WORD_SIZE))),
               unEx(head->head)),
        seq);
  }
...
Tr_exp Tr_ifExp(Tr_exp test, Tr_exp then, Tr_exp elsee) {
  ...
  if (elsee) {
    ...
    */
    ...
    // the expression then and else must have the same type, which is the type
    // of entire if-exp or both produce no value.
    if (elsee->kind == Tr_ex) {
...
Tr_exp Tr_eqStringExp(A_oper op, Tr_exp left, Tr_exp right) {
  ...
  if (op == A_eqOp)
    ...
    T_exp e = (result->kind == T_CONST && result->u.CONST == 1) ? T_Const(0)
                                                                : T_Const(1);
    return Tr_Ex(e);
  }
}
