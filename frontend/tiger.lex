...
/* Add characters to the string one by one */
static void append_to_buffer(char c) {
  int new_length = strlen(string_buffer) + 1;
  /* Double space when the test space is insufficient  */
...
void adjust(void) {
  EM_tokPos=charPos;
  charPos+=yyleng; // yyleng is the length of the token extracted by lex each time 
}
